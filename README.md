...
| `!cat <path>`                    | inside a thread  | Reads a file from the session's working directory and posts it in-thread as a syntax-highlighted code block. Path is resolved against the session cwd; absolute paths and `~` are accepted, relative paths escaping cwd are refused. Files larger than 512 KB are refused; text is chunked at ~11500 chars per Slack markdown block with "Show more" / "Show all" buttons for continuation ("Show all" uploads the raw file). Examples: `!cat src/foo.ts`, `!cat /etc/hosts`. |
